...
class Background:
    ...
    def _populate_grid(self):
        """Adds static background elements to the spatial grid for optimized rendering."""
        # Sun is drawn separately, so not adding it to the general static elements in the grid here.
        ...
        for item in all_static_elements:
            ...
            if 0 <= gy < self.grid_rows and 0 <= gx < self.grid_cols: # Ensure valid indices.
                self.grid[gy][gx].append(item)

    def _is_position_colliding_with_celestial(self, x, y, item_radius):
        """Checks if a given position and radius would overlap with the sun or solar system planets."""
    ...
    def _generate_element_in_world_circle(self, radius_factor=1.0, min_radius_factor=0.0):
        """Generates a random (x, y) position within a specified annulus of the world."""
    ...
    def _generate_garbage_around_point(self, center_x, center_y, object_radius, count):
        """Generates a cluster of garbage items in an annulus around a central point."""
        for _ in range(count):
            for _ in range(10): # Attempts to place.
                min_r = object_radius + MIN_DIST_GARBAGE_FROM_PLANET_SURFACE
                max_r = object_radius + object_radius * PLANET_GARBAGE_ZONE_RADIUS_FACTOR
                angle = random.uniform(0, 2 * math.pi)
                distance = math.sqrt(random.uniform(min_r**2, max_r**2))
                gx = center_x + distance * math.cos(angle)
                gy = center_y + distance * math.sin(angle)
                if not self._is_position_colliding_with_celestial(gx, gy, GARBAGE_SIZE_RANGE[1] / 2.0):
    ...
    def _generate_solar_system_orbiting_planets(self):
        ...
        min_planet_radius, max_planet_radius = 600, 1700
        available_orbital_span = MAX_ORBIT_RADIUS - MIN_ORBIT_RADIUS
        avg_spacing_needed = available_orbital_span / (NUM_SOLAR_SYSTEM_PLANETS if NUM_SOLAR_SYSTEM_PLANETS > 0 else 1)
        ...
        for i in range(NUM_SOLAR_SYSTEM_PLANETS):
            ...
            for _ in range(20): # Attempts to find a non-colliding orbit.
                seg_start = current_orbit_base + i * avg_spacing_needed
                seg_end = current_orbit_base + (i + 1) * avg_spacing_needed - (planet_radius * 2.5)
                seg_end = min(seg_end, MAX_ORBIT_RADIUS - planet_radius)
                seg_start = min(seg_start, seg_end - 100)
                test_r = random.uniform(seg_start, seg_end) if seg_start < seg_end else current_orbit_base + planet_radius
                test_r = max(MIN_ORBIT_RADIUS + planet_radius, test_r)
                test_r = min(MAX_ORBIT_RADIUS - planet_radius, test_r)
                is_too_close = False
                for prev_info in generated_orbit_radii_info:
                    min_dist = prev_info['radius'] + planet_radius + 300 # Buffer ensures separation.
                    if abs(test_r - prev_info['orbit_radius']) < min_dist:
                        is_too_close = True; break
                if not is_too_close: chosen_orbit_radius = test_r; break
            if chosen_orbit_radius == -1: # Fallback placement.
                last_r = generated_orbit_radii_info[-1]['orbit_radius'] + generated_orbit_radii_info[-1]['radius'] if generated_orbit_radii_info else MIN_ORBIT_RADIUS
                chosen_orbit_radius = last_r + planet_radius + random.uniform(300, 600)
            ...
            angle = random.uniform(0, 2 * math.pi)
            speed_num = random.uniform(0.002, 0.005)
            speed_den = 1 + (chosen_orbit_radius / MAX_ORBIT_RADIUS) * 3
            orbit_speed = speed_num / speed_den
            ...
            current_orbit_base = chosen_orbit_radius + planet_radius

    def _generate_general_garbage(self):
        """Scatters general garbage items throughout the world."""
        for _ in range(NUM_GENERAL_GARBAGE):
            for _ in range(10):
                min_r_factor = (SUN_RADIUS + 200) / WORLD_RADIUS
                gx, gy = self._generate_element_in_world_circle(1.0, min_r_factor)
                if not self._is_position_colliding_with_celestial(gx, gy, GARBAGE_SIZE_RANGE[1] / 2.0):
                    self.all_garbage_items.append(Garbage(gx, gy)); break

    def _generate_galactic_band_data(self):
        """Generates a visually dense band of stars, gas, and dust."""
        ...
        for i in range(len(path_points)-1): # Populate gas blobs.
            p1,p2=pygame.math.Vector2(path_points[i]),pygame.math.Vector2(path_points[i+1]); seg_len=p1.distance_to(p2)
            if seg_len==0:continue
            seg_blobs=int((num_gas_blobs/(num_segments if num_segments>0 else 1))*(seg_len/((WORLD_RADIUS*2)/(num_segments if num_segments>0 else 1))))
        ...
        for i in range(len(path_points)-1): # Populate stars in the band.
            p1,p2=pygame.math.Vector2(path_points[i]),pygame.math.Vector2(path_points[i+1]); seg_len=p1.distance_to(p2)
            if seg_len==0:continue
            seg_stars=int((num_band_stars/(num_segments if num_segments>0 else 1))*(seg_len/((WORLD_RADIUS*2)/(num_segments if num_segments>0 else 1))))
            for _ in range(seg_stars):
                ...
                if math.hypot(star_pos.x-WORLD_CENTER_X,star_pos.y-WORLD_CENTER_Y)<=WORLD_RADIUS:
        ...
        for i in range(len(path_points)-1): # Populate dust lanes.
            p1,p2=pygame.math.Vector2(path_points[i]),pygame.math.Vector2(path_points[i+1]); seg_len=p1.distance_to(p2)
            if seg_len==0:continue
            seg_dust=int((num_dust_lanes/(num_segments if num_segments>0 else 1))*(seg_len/((WORLD_RADIUS*2)/(num_segments if num_segments>0 else 1))))
            for _ in range(seg_dust):
                ...
                if math.hypot(dust_pos.x-WORLD_CENTER_X,dust_pos.y-WORLD_CENTER_Y)<=WORLD_RADIUS:
    ...
    def update(self, dt):
        """Updates positions of orbiting planets and resolves garbage-celestial body overlaps."""
        for p_data in self.solar_system_planets:
        ...
        for G_item in self.all_garbage_items: # Simple collision response for garbage.
            sun_dx = G_item.world_x - WORLD_CENTER_X; sun_dy = G_item.world_y - WORLD_CENTER_Y
            sun_dist_sq = sun_dx**2 + sun_dy**2
            if sun_dist_sq < (SUN_RADIUS + G_item.size / 2.0)**2 and sun_dist_sq > 1e-6:
                dist = math.sqrt(sun_dist_sq); overlap = (SUN_RADIUS + G_item.size/2.0)-dist
                push = overlap/dist*0.5; G_item.world_x+=sun_dx*push; G_item.world_y+=sun_dy*push
                G_item.rect.center=(G_item.world_x,G_item.world_y)
            for p_data in self.solar_system_planets:
                p_x,p_y,p_r = p_data['world_pos'][0],p_data['world_pos'][1],p_data['radius']
                dx=G_item.world_x-p_x; dy=G_item.world_y-p_y; dist_sq=dx*dx+dy*dy
                if dist_sq < (p_r + G_item.size / 2.0)**2 and dist_sq > 1e-6:
                    dist=math.sqrt(dist_sq); overlap=(p_r + G_item.size/2.0)-dist
                    push=overlap/dist*0.5; G_item.world_x+=dx*push; G_item.world_y+=dy*push
                    G_item.rect.center=(G_item.world_x,G_item.world_y)

    def draw(self, surface, camera_x, camera_y):
        """Draws all background elements, using the spatial grid for optimization."""
        ...
        start_col = max(0, cam_min_gx)
        end_col = min(self.grid_cols - 1, cam_max_gx)
        start_row = max(0, cam_min_gy)
        end_row = min(self.grid_rows - 1, cam_max_gy)

        # Draw static elements from the grid in layers for correct visual depth.
        # Pass 1: Gas (typically furthest back)
        for gy in range(start_row, end_row + 1):
            for gx in range(start_col, end_col + 1):
                for item in self.grid[gy][gx]:
                    if item.get('type') == 'gas_blob':
        ...
        # Pass 3: Distant Planets (small decorative circles)
        for gy in range(start_row, end_row + 1):
            for gx in range(start_col, end_col + 1):
                for item in self.grid[gy][gx]:
                    if item.get('type') == 'distant_planet':
                        # draw_pixel_circle includes its own culling.
                        draw_pixel_circle(surface, item['color'],
                                          item['world_pos'][0] - camera_x,
                                          item['world_pos'][1] - camera_y,
                                          item['radius'])
            ...
            for gx in range(start_col, end_col + 1):
                for item in self.grid[gy][gx]:
                    if item.get('type') == 'star':
                        # draw_pixel_star includes its own culling.
                        draw_pixel_star(surface,
                                        item['world_pos'][0] - camera_x,
                                        item['world_pos'][1] - camera_y,
                                        item['color'], item['size_cat'])

        # Draw Solar System Planets (dynamic elements, not solely from grid for position).
        ...
        # Draw Sun (typically on top of other background elements).
